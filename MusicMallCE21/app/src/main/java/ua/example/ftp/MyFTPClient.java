...
public class MyFTPClient {
	...
	public boolean ftpChangeDirectory(String directory_path)
	{
	    ...
	    try {
	        ...
	        stat=mFTPClient.changeWorkingDirectory(directory_path);
	    } catch(Exception e) {
	        Log.d(TAG, "Error: could not change directory to " + directory_path);
	    }
	    if(!stat)
		    {
	    	Log.d(TAG, "Try № "+count+" Failed change dir ftp,wait 500 nanoSecund :" + directory_path);
				try {
					Thread.sleep(500);
	    	...
	    	{
	    	Log.d(TAG, "Try № "+count+ " Success change dir ftp :" + directory_path);
	    	return stat;
	    	}
	    count++;
		}
	...
	public boolean ftpMakeDirectory(String new_dir_path)
	{
		...
		int count =1;
		if (!ftpChangeDirectory(new_dir_path))
		{
			    ...
			    if(!status)
			    {
		    	Log.d(TAG, "Try № "+count+" Failed create new directory ftp,wait 500 nanoSecund :" + new_dir_path);
					try {
						Thread.sleep(500);
					} catch (InterruptedException e) {
		    	...
		    	{
		    	Log.d(TAG, "Try № "+count+ " Success create new directory ftp :" + new_dir_path);
		    	return status;
		    	}
		    count++;
			}//while
		...
		{
			 Log.d(TAG, "Каталог "+ new_dir_path+" есть,создавать не нужно!" );
		}
	 return status;
	} 
	...
	public boolean ftpDownload(String srcFilePath, String desFilePath)
	{
	    boolean status = false;
	    try {
	    	 File folder = new File(desFilePath);
	    	 folder.getParentFile().mkdirs();
	    	
	        FileOutputStream desFileStream = new FileOutputStream(folder);
	    ...
	    {
	        Log.d(TAG, "download failed"+" erorr:"+e);
	    }
